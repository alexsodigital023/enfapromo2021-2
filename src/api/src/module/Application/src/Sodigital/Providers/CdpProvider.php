<?php
namespace Application\Sodigital\Providers;
...
class CdpProvider{
     ...
     * @param Adapter $db
     */
    function __construct($config, Adapter &$db)
    {
        $this->config=$config;
        $this->db=&$db;
    }
    ...
    protected function storeToken($token){
        ...
        $this->db->query('delete from `cdp_token` WHERE `expires_date` < ?', [date("Y-m-d H:i:s")]);

        $insert=$sql->insert("cdp_token")
    ...
    protected function getTokenFromDatabase(){
        $r=$this->db->query('select * from `cdp_token` WHERE `expires_date` > ? limit 1', [date("Y-m-d H:i:s")]);
        if(!$r->count()){
            $r=$this->db->query('select * from `cdp_token` WHERE `expires_date` < ? limit 1', [date("Y-m-d H:i:s")]);
            if($r->count()){
